...
export default function AddPartai() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    formData.append('nama', partai.nama);
    formData.append('ketua_umum', partai.ketua_umum);
    formData.append('visi_misi', partai.visi_misi);
    formData.append('alamat', partai.alamat);
    formData.append('image', partai.image);

    setIsLoading(true);
    const res = await fetch('http://localhost:3000/api/v1/partai/add', {
      method: 'POST',
      headers: {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPartai(prev => ({ ...prev, image: e.target.files ? e.target.files[0] : e.target.value }));
    }
  };
  ...
  return (
    <div>
      ...
      <h1 className="text-4xl font-bold text-center my-10">ADD PARTAI</h1>
          ...
          <div className="w-full h-5/6 flex flex-col justify-between">
            <div className="flex flex-col">
              ...
              <input
                type="text"
                id="name"
                onChange={e => setPartai(prev => ({ ...prev, nama: e.target.value }))}
            ...
            <div className="flex flex-col mt-5">
              ...
              <input
                type="text"
                id="name"
                onChange={e => setPartai(prev => ({ ...prev, ketua_umum: e.target.value }))}
                className=" border-2 rounded-lg focus:outline-none py-1 px-3 focus:shadow focus:border-gray-300"
              />
            </div>
            <div className="flex flex-col mt-5">
              ...
              <input
                type="text"
                id="name"
                onChange={e => setPartai(prev => ({ ...prev, alamat: e.target.value }))}
                className=" border-2 rounded-lg focus:outline-none py-1 px-3 focus:shadow focus:border-gray-300"
              />
            </div>
            <div className="flex flex-col mt-5">
              ...
              <textarea
                ...
                cols={30}
                rows={10}
                onChange={e => setPartai(prev => ({ ...prev, visi_misi: e.target.value }))}
